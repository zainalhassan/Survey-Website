...
elseif (isset($_POST['username']))
{
  ...
  //the following line will validate the email as a string, but maybe you can do a better job...
	...
	$errors = $username_val . $password_val . $email_val . $firstName_val . $lastName_val . $dob_val . $telephone_val . $dob_val . $telephone_val;
	...
	if ($errors == "")
	{
		...
		$query = "INSERT INTO users (firstName, lastName, username, password, email, dob, telephone) VALUES ('$firstName', '$lastName', '$username', '$password', '$email', '$dob', '$telephone');";
...
<form action="sign_up.php" method="POST">
  ...
  <br>
  Password: <input type="password" name="password" minlength = "6" maxlength="16" value="$password" required> $password_val
  <br>
  Email: <input type="email" name="email" minlength = "1" maxlength="64" value="$email" required> $email_val
  <br>
  First Name: <input type="fistName" name="firstName" minlength = "1" maxlength="20" value="$firstName" required> $firstName_val
  <br>
  Last Name: <input type="lastName" name="lastName" minlength = "1" maxlength="30" value="$lastName" required> $lastName_val
  <br>
  Date of Birth: <input type="date" name="dob" maxlength = "10"  max = "$dateRestriction" value="$dob" required> $dob_val
  <br>
  Telephone: <input type="telephone" name="telephone" minlength = "1" maxlength="16" value="$telephone" required> $telephone_val
  <br>
